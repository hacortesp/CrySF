...
def parse_args():
    ...
    parser.add_argument('-to'  , type=str,   default='npt.data', help='Name of topology file (.gro or .data formats)')
    parser.add_argument('-tr'  , type=str,   default='traj.xtc', help='Name of trajectory file')
    parser.add_argument('-f'   , type=str,   default='XTC', help='Format options: XTC, TRR, LAMMPS, XDATCAR')
    parser.add_argument('-ts'  , type=float, defailt=0.25, help='Time interval between frames in the trajectory')
    parser.add_argument('-v'   , type=float, default=0.2, help='Voxel size')
    parser.add_argument('-a'   , type=str,   default= 4, help='Atom selection: integer (index) or string (name) based on trajectory format')
